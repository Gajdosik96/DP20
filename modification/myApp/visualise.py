...
from myApp.filters import IFilter, MasterFilter, ColorizerFilter
from myApp.arch_generator import gen_inference_deep, gen_inference_wide
...
class CustomVisualiser(object):
    def __init__(self):
        pass

    def plot_compare2generators(self, myLearner: Learner, deoldifyLearner: Learner, img: Image):
        figsize = (20, 20)

        my_image = image2np(myLearner.predict(img)[2])
        deoldify_image = image2np(deoldifyLearner.predict(img)[2])

        self._plot_comparison_custom(
            figsize, my_image, deoldify_image
        )

    def _plot_comparison_custom(
            self,
            figsize: (int, int),
            my_image: Image,
            deoldify_image: Image
    ):
        fig, axes = plt.subplots(1, 2, figsize=figsize)
        self._plot_image_custom(
            my_image,
            "Mine",
            axes=axes[0],
            figsize=figsize
